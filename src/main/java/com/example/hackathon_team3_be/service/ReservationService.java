...
public class ReservationService {
    ...
    public ReservationResponse reserve(ReservationRequest request) {
        if (request.scheduledAt().getMinute() != 0
                || request.scheduledAt().getHour() < 11
                || request.scheduledAt().getHour() > 18) {
        ...
        if (reservationRepository.findBySessionId(session.getId()).isPresent()) {
